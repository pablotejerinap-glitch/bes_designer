"""
Migración de catálogos JSON → Excel (.xlsx)
============================================

Lee los catálogos JSON existentes en ``catalogs/`` y genera un archivo
Excel por tipo de equipo en ``database_migration/data_excel/``.

POR QUÉ EXISTE ESTE SCRIPT
--------------------------
El objetivo del proyecto es que la información de ingeniería viva fuera
del código fuente, en un formato que un Ingeniero de Petróleo pueda
editar y validar sin programar. Excel es ese formato durante el
desarrollo; más adelante la misma estructura migrará a SQLite.

DISEÑO
------
* Un archivo .xlsx por tipo de equipo → agregar un catálogo nuevo es
  agregar un archivo, sin tocar código.
* Datos anidados (curvas de bomba, caída de tensión de cables) van en
  una hoja de detalle ligada a la hoja maestra por una clave legible
  (ej. ``pump_id = "Reda_400_D-40"``). Es el mismo esquema
  maestro→detalle que usarán las tablas SQLite.
* Listas simples (housing_options, compatible_motor_series) se guardan
  como texto separado por comas: fácil de editar a mano.
* La columna ``source`` preserva la trazabilidad de cada dato
  (Brown Vol. 2B, catálogos de fabricante, etc.).
* Cada archivo incluye una hoja README con la descripción de cada
  columna y sus unidades, en español.

USO
---
    python migrate_json_to_excel.py

No modifica nada del proyecto original: solo LEE los JSON y ESCRIBE
en database_migration/data_excel/.
"""
...
# --- Rutas (relativas a este archivo: funciona en cualquier PC) -------
BASE_DIR = Path(__file__).parent            # database_migration/
CATALOG_DIR = Path(bes.__file__).parent / "catalogs"  # catalogos del paquete bes
OUTPUT_DIR = BASE_DIR / "data_excel"

# --- Estilos ----------------------------------------------------------
FONT = "Arial"
HEADER_FONT = Font(name=FONT, bold=True, color="FFFFFF")
HEADER_FILL = PatternFill("solid", start_color="1F4E79")  # azul oscuro
CELL_FONT = Font(name=FONT)


def make_id(entry: dict) -> str:
    """Clave legible: fabricante_serie_modelo (ej. 'Reda_400_D-40').

    Se usa clave legible (no un número) para que la hoja de detalle
    pueda leerse sin cruzar tablas.
    """
...
def write_sheet(ws, headers: list[str], rows: list[list]) -> None:
    """Escribe encabezado con estilo + filas de datos en una hoja."""
    ws.append(headers)
    for cell in ws[1]:
    ...
    for row in ws.iter_rows(min_row=2):
        for cell in row:
            cell.font = CELL_FONT
    # ancho de columna según contenido
...
def write_readme(wb, title: str, descriptions: list[tuple[str, str]]) -> None:
    """Hoja README: descripción de cada columna, en español."""
...
# ======================================================================
# BOMBAS: hoja maestra + hoja de curvas de rendimiento
# ======================================================================
...
# ======================================================================
# CABLES: hoja maestra + hoja de caída de tensión vs temperatura
# ======================================================================
def migrate_cables() -> None:
    ...
    # cable_id incluye el CONDUCTOR (CU/AL): existen cables del mismo
    # tipo y calibre en cobre y aluminio, con caídas de tensión muy
    # distintas. Sin el conductor, la clave colisiona.
...
# ======================================================================
# Catálogos planos: motores, sellos, gas handlers, sensores
# ======================================================================
def migrate_flat(json_name: str, key: str, out_name: str, columns: list[str],
                 title: str, descriptions: list[tuple[str, str]],
                 list_columns: tuple[str, ...] = ()) -> None:
    """Migra un catálogo tabular simple (una fila por equipo).

    ``list_columns``: columnas que en JSON son listas y en Excel se
    ...
    for e in entries:
        row = []
        for col in columns:
            # las columnas de trazabilidad se llaman _source/_range_source en JSON
