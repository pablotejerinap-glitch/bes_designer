...
1. **Schema** (`bes/api/schemas/`): definir el/los modelo(s) Pydantic de
   request y response que espejan las dataclasses relevantes de `bes/core/models.py`.
   Enums como **string** (nunca los enteros de `auto()`).
2. **Mapper** (`bes/api/mappers.py`): mapear schema↔dataclass.
...
5. **Gráficos**: si el endpoint devuelve un gráfico, llamá al builder de
   `bes.plotting` y devolvé `fig.to_json()`. No reimplementar en el router.
